...
import pncp_backend  # Certifique-se de atualizar essa lib para aceitar os novos parêmetros
...
with col_filtros:
    with st.form("filtros_pncp"):
        ...
        with st.expander("Filtros principais", expanded=True):
            ...
            # --- ALTERAÇÃO: INÍCIO DOS CAMPOS DE VALOR ---
            st.markdown("---")
            st.markdown("**Faixa de Valor Unitário Estimado (Opcional)**")
            c_val1, c_val2 = st.columns(2)
            with c_val1:
                valor_min_str = st.text_input(
                    "Valor Mínimo (R$)",
                    placeholder="Ex.: 100,00",
                    help="Use vírgula para decimais (ex: 1500,50)."
                )
            with c_val2:
                valor_max_str = st.text_input(
                    "Valor Máximo (R$)",
                    placeholder="Ex.: 5000,00",
                    help="Use vírgula para decimais."
                )
            st.markdown("---")
            # --- ALTERAÇÃO: FIM DOS CAMPOS DE VALOR ---
...
with col_ajuda:
    ...
    st.markdown(
        ...
        <ol style="padding-left: 1.2rem;">
          ...
          <li>Se precisar, filtre por <strong>faixa de valor</strong> (Min/Max).</li>
...
# --- ALTERAÇÃO: FUNÇÃO PARA CONVERTER MOEDA (STRING PARA FLOAT) ---
def _parse_money_or_none(text, campo_nome):
    """
    Converte texto formato BR (1.000,00) para float (1000.00).
    """
    if not text:
        return None
    ...
    try:
        ...
        st.warning(f"Valor inválido em '{campo_nome}'. Use formato numérico (ex: 1500,00). Ignorando filtro.")
        return None
...
if executar:
    ...
    # --- ALTERAÇÃO: CONVERSÃO DOS VALORES ---
    val_min_float = _parse_money_or_none(valor_min_str, "Valor Mínimo")
    val_max_float = _parse_money_or_none(valor_max_str, "Valor Máximo")

    # Converte selects booleanos
    tem_resultado = _opt_to_bool(tem_resultado_opt, "Somente com resultado", "Somente sem resultado")
    bps = _opt_to_bool(bps_opt, "Somente BPS verdadeiro", "Somente BPS falso")
    mpn = _opt_to_bool(mpn_opt, "Somente com margem", "Somente sem margem")
    ...
    with st.spinner("Consultando API do PNCP e gerando arquivos..."):
        # --- ALTERAÇÃO: PASSAGEM DOS NOVOS PARÂMETROS PARA O BACKEND ---
        # Nota: Você deve atualizar a definição da função no arquivo pncp_backend.py
        # para receber valor_min e valor_max
        excel_bytes, html_string, meta = pncp_backend.executar_pesquisa_e_gerar_arquivos(
            ...
            valor_min=val_min_float,
            valor_max=val_max_float,
